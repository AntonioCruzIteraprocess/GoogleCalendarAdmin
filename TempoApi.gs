...
function postTempoWorklog(eventData, classification) {
  ...
  const response = UrlFetchApp.fetch("https://api.tempo.io/4/worklogs", {
    method: "post",
  ...
  if (code !== 200 && code !== 201) throw new Error("Tempo error " + code + ": " + response.getContentText().substring(0, 200));
  return JSON.parse(response.getContentText());
}

function getTempoWorklogs(fromDate, toDate) {
  const tc = getTempoConfig();
  let results = [], offset = 0, limit = 50;

  do {
    const url = "https://api.tempo.io/4/worklogs/user/" + tc.tempo_account_id
    ...
    const code = response.getResponseCode();
    if (code !== 200) throw new Error("Tempo error " + code + ": " + response.getContentText().substring(0, 200));
...
function deleteTempoWorklog(tempoWorklogId) {
  const tc = getTempoConfig();
  const response = UrlFetchApp.fetch("https://api.tempo.io/4/worklogs/" + tempoWorklogId, {
    method: "delete",
    headers: { "Authorization": "Bearer " + tc.tempo_token, "Accept": "application/json" },
    muteHttpExceptions: true
  });
  const code = response.getResponseCode();
  if (code !== 200 && code !== 204) throw new Error("Delete error " + code);
}
...
function fetchIssueDetailsById_(ids) {
  ...
  for (let i = 0; i < ids.length; i += batchSize) {
    ...
    const response = UrlFetchApp.fetch(tc.jira_server + "/rest/api/3/search/jql", {
      ...
      payload: JSON.stringify({ jql: jql, maxResults: batchSize, fields: ["summary", "parent"] }),
      muteHttpExceptions: true
    });
    if (response.getResponseCode() === 200) {
      ...
      (data.issues || []).forEach(issue => {
        details[issue.id] = {
          ...
          epicName: issue.fields.parent ? (issue.fields.parent.fields.summary || issue.fields.parent.key) : ""
        };
      });
    }
  }
